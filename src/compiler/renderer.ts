...
export function renderBrief(ctx: ResolvedContext): string {
  ...
  if (ctx.sources.length === 0) {
    ...
    for (const s of ctx.sources) {
      if (ctx.verbosity === "detailed") {
        lines.push(
          `### ${s.type.toUpperCase()}: ${s.path.split(/[\\/]/).pop()}`,
        );
        lines.push("```text");
  ...
  if (ctx.mode === "RESEARCH") {
    ...
    lines.push(
      "- **Questions & Synthesis**: Identify authoritative sources and answer core questions.",
    );
    lines.push(
      "- **Uncertainty**: State missing information clearly. Do not guess.",
    );
    ...
    lines.push(
      "- **Output**: Write a step-by-step `BUILD_PLAN.md` with implementation batches. Do NOT modify source code.",
    );
  } else if (ctx.mode === "BUILD") {
    lines.push("## Build Directives");
    lines.push(
      "- **Scope**: Execute exactly ONE vertical slice or unchecked plan item.",
    );
    lines.push("- **Files**: Make the smallest coherent diff required.");
    lines.push(
      "- **Evidence**: You MUST run validation tests before completion claims.",
    );
    lines.push(
      "- **Stop Condition**: If tests fail twice with the same error, STOP.",
    );
  } else if (ctx.mode === "REVIEW") {
    lines.push("## Review Directives");
    lines.push(
      "- **Inspect**: Check the diffs for syntax, logic, and architectural defects.",
    );
    ...
    lines.push(
      "- **Avoid**: Do not repeat work that is already merged or verified.",
    );
  }
  ...
  if (ctx.conflictsSurfaced.length > 0) {
    ...
    for (const c of ctx.conflictsSurfaced) {
      lines.push(
        `- **Conflict**: \`${c.overwrittenId}\` vs \`${c.winnerId}\`. **Resolution**: ${c.reason}`,
      );
    }
  }
